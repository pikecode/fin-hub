...
- 银行流水保留 `ledger_period` 作为发生账期，默认由 `occurred_at` 推导；它用于流水列表、默认筛选、银行流水趋势和未匹配流水统计。
...
- 支出匹配和收入匹配均以匹配记录的 `accounting_period` 作为对账/入账归属账期，银行流水自身的 `ledger_period` 只表示发生月份。
- 后续智能推荐算法只生成候选，不直接确认。
